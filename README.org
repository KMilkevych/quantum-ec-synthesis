...
#+RESULTS:
#+begin_example
usage: main.py generate [-h] [-k {all,identity,x,h,snake,entangle,random-clifford}]
                        [-q QUBITS] [-gc GATE_COUNT] [-f {qasm2,qasm3}] [-o OUTPUT]
...
options:
  ...
  -o, --output OUTPUT   output file/directory for storing output circuits (default:
                        None)
#+end_example
